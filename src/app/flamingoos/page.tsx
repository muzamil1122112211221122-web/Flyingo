...
export default function FlamingoosPage() {
  ...
  const gradientOptions = [
    { label: "Flamingo Sunset", bg: "linear-gradient(135deg, #b70942, #da2e59)" },
  ...
  return (
    <div className="min-h-screen bg-surface flex text-on-surface">
      ...
      <div className="md:ml-[72px] ml-0 flex-1 flex flex-col p-4 md:p-6 lg:p-10 max-w-7xl mx-auto w-full pb-24 md:pb-8">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[28px] text-primary">auto_stories</span>
              <h1 className="font-display-lg font-bold tracking-tight">Flamingoos</h1>
            </div>
            <p className="font-body-md text-on-surface-variant mt-1">
              Upload custom photos, video clips, or thoughts. Ephemeral 2-minute updates.
            </p>
          </div>
          ...
          <motion.button
            ...
            className="px-6 py-3 rounded-2xl bg-gradient-to-r from-primary to-secondary text-white font-title-md font-semibold flex items-center gap-2 shadow-lg shadow-primary/20 self-start sm:self-auto cursor-pointer"
          >
        ...
        {/* Flamingoos Horizontal Tray */}
        <div className="p-6 rounded-3xl bg-surface-container-lowest shadow-sm mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-title-md font-bold text-on-surface flex items-center gap-2">
              <span>Recent Flamingoos</span>
              {stories.length > 0 && (
                <span className="px-2 py-0.5 rounded-full bg-primary/10 text-primary font-caption font-bold text-[10px]">{stories.length} ACTIVE</span>
              )}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-6 overflow-x-auto pb-2 scrollbar-none">
            ...
            <div className="flex flex-col items-center gap-2 flex-shrink-0 cursor-pointer group" onClick={() => setShowCreateModal(true)}>
              <div className="relative">
                <div className="w-18 h-18 sm:w-20 sm:h-20 rounded-full p-1 bg-surface-container flex items-center justify-center group-hover:scale-105 transition-transform shadow-md">
                  <div className="w-full h-full rounded-full bg-gradient-to-tr from-secondary to-primary flex items-center justify-center text-white font-bold text-lg">
                    {(currentUser.name || currentUser.handle || "U").substring(0, 2).toUpperCase()}
                  </div>
                </div>
                <span className="absolute bottom-0 right-0 w-6 h-6 rounded-full bg-primary text-white flex items-center justify-center font-bold text-sm shadow-md ring-2 ring-surface-container-lowest">
                  +
                </span>
              </div>
              <span className="font-label-md text-center text-on-surface text-[12px]">Your Flamingoo</span>
            </div>

            {/* Grouped Stories from users */}
            {groupedStories.map((group) => {
              ...
              return (
                ...
                >
                  <div className="relative">
                    <div className="w-18 h-18 sm:w-20 sm:h-20 rounded-full p-1 bg-gradient-to-tr from-primary via-secondary to-tertiary flex items-center justify-center group-hover:scale-105 transition-transform shadow-md">
                      <div className="w-full h-full rounded-full bg-surface-container-lowest p-1">
                        {firstStory.type === "image" && firstStory.mediaUrl ? (
                          ...
                          <div className="w-full h-full rounded-full flex items-center justify-center text-white font-bold text-lg" style={{ background: firstStory.background }}>
                    ...
                    {storyCount > 1 ? (
                      <span className="absolute -top-1 -right-1 px-1.5 py-0.5 rounded-full bg-secondary text-white text-[10px] font-bold shadow-md ring-2 ring-surface-container-lowest">
                        {storyCount}
                      </span>
                    ) : (
                  ...
                  <div className="flex flex-col items-center">
                    <span className="font-label-md text-center text-on-surface truncate max-w-[80px] text-[12px]">{firstStory.authorName}</span>
                    <span className="font-caption text-on-surface-variant text-[10px]">{firstStory.timestamp}</span>
                  </div>
                </div>
              );
            })}
        ...
        {/* Flamingoos Feed / Clean Empty State */}
        {stories.length === 0 ? (
          <div className="p-12 rounded-3xl bg-surface-container-lowest shadow-sm flex flex-col items-center justify-center text-center">
            ...
            <h3 className="font-headline-sm font-bold text-on-surface mb-2">No Flamingoos Posted Yet</h3>
            <p className="font-body-md text-on-surface-variant max-w-md mb-6">
              Share your custom photo, video, or thoughts. Only real flamingoos will appear here!
            </p>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            ...
            {stories.map((story, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="h-80 rounded-3xl p-6 flex flex-col justify-between shadow-xl cursor-pointer relative overflow-hidden text-white transition-all"
                  style={{ background: story.mediaUrl ? "#131b2e" : story.background }}
                >
                  {/* If image story */}
                  {story.type === "image" && story.mediaUrl && (
                    <img src={story.mediaUrl} alt="" className="absolute inset-0 w-full h-full object-cover -z-0 opacity-80" />
                  )}
                  ...
                  <div className="flex items-center justify-between z-10">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-title-md font-bold leading-tight drop-shadow">{story.authorName}</p>
                        <p className="font-caption text-white/80 drop-shadow">@{story.authorHandle} • {story.timestamp}</p>
                      </div>
                    </div>
                    <span className="px-2.5 py-1 rounded-full bg-black/40 backdrop-blur-md text-caption font-semibold text-[11px]">
                      {story.durationSeconds}s
                    </span>
                  </div>

                  {/* Moveable Story content */}
                  <div 
                    className="z-10 w-full"
                    style={{
                      marginTop: story.textYOffset !== undefined ? `${(story.textYOffset / 100) * 120}px` : "auto",
                      marginBottom: "auto",
                    }}
                  >
                    {story.content && (
                      <p 
                        className={`leading-snug drop-shadow-md break-words p-3 rounded-2xl ${fontClasses[story.fontFamily || "sans"] || "font-sans"} ${story.isBold ? "font-bold" : "font-normal"} ${story.isItalic ? "italic" : ""} ${story.isUnderline ? "underline" : ""} bg-black/40 backdrop-blur-xs`}
                        style={{ fontSize: "17px" }}
                      >
                        {story.content}
                      </p>
                  ...
                  {/* Footer: Likes, and Views ONLY for your own story */}
                  <div className="flex items-center justify-between z-10 pt-4 border-t border-white/20">
                    <div className="flex items-center gap-3">
                      <button
                        onClick={(e) => { e.stopPropagation(); handleLike(story.id); }}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/40 hover:bg-black/60 backdrop-blur-md transition-colors font-label-md text-[12px]"
                      >
                        <span className="material-symbols-outlined text-[16px] text-red-400" style={{ fontVariationSettings: story.hasLiked ? "'FILL' 1" : "'FILL' 0" }}>
                          favorite
                        </span>
                        <span>{story.likes}</span>
                      </button>

                      {/* Views only visible if it's YOUR story */}
                      {isMine && (
                        <div className="flex items-center gap-1 px-3 py-1.5 rounded-full bg-black/40 backdrop-blur-md font-label-md text-[12px] text-tertiary-fixed">
                    ...
                    <span className="font-caption text-white/80 text-[11px] flex items-center gap-1">
                      Tap to view <span className="material-symbols-outlined text-[14px]">arrow_forward</span>
                    </span>
                  </div>
                </motion.div>
              );
      ...
      {/* ── CREATE FLAMINGOO MODAL (MOVEABLE TEXT + FONTS + BOLD/ITALIC/UNDERLINE) ── */}
      <AnimatePresence>
        {showCreateModal && (
          <motion.div
          ...
          >
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-5 flex flex-col gap-4">
                ...
                {/* Story Type Selector — icon+short label, equal flex */}
                ...
                {/* Interactive Live Flamingoo Canvas Preview */}
                <div
                ...
                >
                  ...
                  <div className="flex items-center justify-between z-10">
                    ...
                    {mediaDataUrl && (
                      <button
                        type="button"
                        onClick={() => { setMediaDataUrl(null); setStoryType("text"); }}
                        className="px-2.5 py-1 rounded-xl bg-error/80 text-white font-caption text-[11px] cursor-pointer"
                      >
                        Remove
                      </button>
                    )}
                  ...
                  <div className="z-10 w-full flex-1 flex items-center justify-center relative overflow-visible">
                    {!textDeleted && newStoryText ? (
                      <motion.div
                        drag
                        dragConstraints={{ left: -140, right: 140, top: -75, bottom: 75 }}
                        dragElastic={0.1}
                        dragMomentum={false}
                      ...
                      >
                        <p
                          className={`leading-relaxed inline-block p-3 rounded-2xl max-w-full break-words shadow-xl ${fontClasses[fontFamily]} ${isBold ? "font-bold" : "font-normal"} ${isItalic ? "italic" : ""} ${isUnderline ? "underline" : ""} ${textHighlight ? "bg-black/60 backdrop-blur-md" : "drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)]"}`}
                          style={{ fontSize: "18px" }}
                        >
                          {newStoryText}
                        </p>
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); setTextDeleted(true); }}
                          className="w-6 h-6 rounded-full bg-error/90 hover:bg-error text-white flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer shrink-0"
                          title="Remove text"
                        >
                      ...
                      <button
                        type="button"
                        onClick={() => setTextDeleted(false)}
                        className="px-3 py-1.5 rounded-xl bg-black/50 text-white/80 hover:text-white font-caption text-xs flex items-center gap-1 cursor-pointer"
                      >
                ...
                <div className="p-3.5 rounded-2xl bg-surface-container-low flex flex-col gap-3">
                    ...
                    <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 flex-1">
                      {[
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-xl text-[11px] whitespace-nowrap transition-all cursor-pointer shrink-0 ${
                            fontFamily === f.id ? "bg-primary text-white font-bold shadow-xs" : "bg-surface-container text-on-surface-variant hover:bg-surface-container-high"
                          }`}
                        >
                          {f.label}
                        </button>
                  ...
                  {/* B / I / U + Pill + Drag hint */}
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center gap-1.5">
                      <button
                        type="button"
                        onClick={() => setIsBold(!isBold)}
                        className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-sm cursor-pointer transition-colors ${isBold ? "bg-primary text-white" : "bg-surface-container text-on-surface-variant"}`}
                        title="Bold"
                      >B</button>
                      <button
                        type="button"
                        onClick={() => setIsItalic(!isItalic)}
                        className={`w-8 h-8 rounded-xl flex items-center justify-center italic font-serif text-sm cursor-pointer transition-colors ${isItalic ? "bg-primary text-white" : "bg-surface-container text-on-surface-variant"}`}
                        title="Italic"
                      >I</button>
                      <button
                        type="button"
                        onClick={() => setIsUnderline(!isUnderline)}
                        className={`w-8 h-8 rounded-xl flex items-center justify-center underline text-sm cursor-pointer transition-colors ${isUnderline ? "bg-primary text-white" : "bg-surface-container text-on-surface-variant"}`}
                        title="Underline"
                      >U</button>
                      <button
                        type="button"
                        onClick={() => setTextHighlight(!textHighlight)}
                        className={`px-2.5 py-1 h-8 rounded-xl flex items-center gap-1 text-[11px] cursor-pointer transition-colors ${textHighlight ? "bg-secondary text-white" : "bg-surface-container text-on-surface-variant"}`}
                    ...
                    <div className="flex items-center gap-2">
                      <span className="font-caption text-[10px] text-on-surface-variant flex items-center gap-0.5">
                        <span className="material-symbols-outlined text-[13px] text-primary">pan_tool</span>
                        Drag text
                      </span>
                      {(dragOffset.x !== 0 || dragOffset.y !== 0) && (
                        <button
                          ...
                          className="px-2 py-0.5 rounded-lg bg-surface-container text-on-surface-variant hover:text-on-surface text-[10px] font-caption cursor-pointer"
                        >
                          Reset
                        </button>
                      )}
        ...
        {activeStory && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-md h-[85vh] max-h-[750px] rounded-3xl p-6 flex flex-col justify-between shadow-2xl overflow-hidden text-white select-none"
              style={{ background: activeStory.mediaUrl ? "#0c0d18" : activeStory.background }}
            >
              {/* Media element background if photo/video */}
              {activeStory.type === "image" && activeStory.mediaUrl && (
                <img src={activeStory.mediaUrl} alt="" className="absolute inset-0 w-full h-full object-contain -z-0" />
              )}
              {activeStory.type === "video" && activeStory.mediaUrl && (
                <video src={activeStory.mediaUrl} autoPlay loop playsInline className="absolute inset-0 w-full h-full object-contain -z-0" />
              )}
              ...
              {/* Story Viewer Header */}
              <div className="flex items-center justify-between mt-4 z-30 bg-black/30 backdrop-blur-xs p-2 rounded-2xl">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center font-bold shadow-inner">
                    {activeStory.authorAvatar}
                  </div>
                  <div>
                    <p className="font-title-md font-bold drop-shadow leading-tight">{activeStory.authorName}</p>
                    <p className="font-caption text-white/80 drop-shadow">@{activeStory.authorHandle} • {activeStory.timestamp}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="w-8 h-8 rounded-full bg-black/40 flex items-center justify-center hover:bg-black/60 transition-colors cursor-pointer"
                  >
              ...
              {/* Story Middle Canvas (Custom Moveable Styled Text) */}
              <div 
                className="z-20 px-4 text-center my-auto transition-all"
                style={{
                  transform: (activeStory.textXOffset !== undefined || activeStory.textYOffset !== undefined)
                    ? `translate(${activeStory.textXOffset || 0}px, ${activeStory.textYOffset || 0}px)`
                    : "none",
                }}
              >
                {activeStory.content && (
              ...
              <div
                className="absolute left-0 top-20 bottom-28 w-1/3 z-20 cursor-pointer"
                onClick={() => {
                  if ((activeStoryIndex || 0) > 0) {
                    ...
                    if (currentGroupIdx > 0) {
                      ...
                      const prevHandle = prevGroup[0]?.authorHandle || prevGroup[0]?.userId || "";
                      setActiveUserHandle(prevHandle);
                      setActiveStoryIndex(prevGroup.length - 1);
                      setStoryProgress(0);
                    }
                  }
                }}
              />
              <div
                className="absolute right-0 top-20 bottom-28 w-1/3 z-20 cursor-pointer"
                onClick={() => {
                  if ((activeStoryIndex || 0) < activeUserStories.length - 1) {
                    setActiveStoryIndex((activeStoryIndex || 0) + 1);
                    setStoryProgress(0);
                  } else {
              ...
              {/* Story Viewer Footer: Likes, Reply, and AUTHOR-ONLY Views Drawer */}
              <div className="flex flex-col gap-3 z-30">
                {replySentToast && (
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="p-2 rounded-xl bg-black/70 backdrop-blur-md text-center font-caption text-tertiary-fixed"
                  >
                    ✓ Reply sent to @{activeStory.authorHandle}'s direct chat!
                  </motion.div>
                )}

                {/* Author-only Viewers pill & details */}
                {isMyStory && (
                  <div className="flex items-center justify-between bg-black/50 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/10">
                    <button
                      ...
                      className="flex items-center gap-1.5 text-caption font-semibold text-tertiary-fixed hover:underline cursor-pointer"
                    >
                      <span className="material-symbols-outlined text-[18px]">visibility</span>
                      <span>{(activeStory.viewers || []).length} Viewers (Only you can see this)</span>
                    </button>
                    <span className="text-[12px] opacity-70">
                      {activeStory.likes} Likes
                    </span>
                  </div>
                )}

                {/* Author's Viewers List Drawer */}
                <AnimatePresence>
                  {isMyStory && showViewersDrawer && (
                    ...
                    >
                      <p className="font-caption text-white/60 text-[10px] uppercase font-bold">People who viewed your flamingoo:</p>
                      {(activeStory.viewers || []).length === 0 ? (
                        <p className="font-caption text-white/50 text-[11px]">No views recorded yet</p>
                      ) : (
                ...
                <div className="flex items-center gap-3">
                  {!isMyStory ? (
                    <div className="flex-1 flex items-center bg-black/40 backdrop-blur-md rounded-full px-4 py-2.5 border border-white/20">
                      ...
                      />
                      {replyText.trim() && (
                        <button onClick={handleSendReply} className="text-white hover:text-primary cursor-pointer">
                  ...
                  >
                    <span
                      ...
                      style={{ fontVariationSettings: activeStory.hasLiked ? "'FILL' 1" : "'FILL' 0" }}
                    >
                      favorite
                    </span>
                  </motion.button>
